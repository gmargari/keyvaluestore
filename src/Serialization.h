...
bool     serialize(Buffer *buf, const char *key, uint32_t keylen, const char *value, uint32_t valuelen, uint64_t timestamp, uint32_t *len);
...
bool     deserialize(Buffer *buf, const char **key, const char **value, uint64_t *timestamp, uint32_t *len);
