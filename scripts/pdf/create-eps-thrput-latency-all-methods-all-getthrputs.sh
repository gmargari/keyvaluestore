...
#ymax2=`cat $file_imm $file_gp2 $file_rng $file_geo $file_cas 2> /dev/null | awk '{if ($2 > max) max = $2;} END{print max*1.04}'`
#ymax3=`cat $file_imm $file_gp2 $file_rng $file_geo $file_cas 2> /dev/null | awk '{if ($3 > max) max = $3;} END{print max*1.04}'`
#ymax4=`cat $file_imm $file_gp2 $file_rng $file_geo $file_cas 2> /dev/null | awk '{if ($4 > max) max = $4;} END{print max*1.04}'`
#ymax5=`cat $file_imm $file_gp2 $file_rng $file_geo $file_cas 2> /dev/null | awk '{if ($5 > max) max = $5;} END{print max*1.04}'`
#ymax7=`cat $file_imm $file_gp2 $file_rng $file_geo $file_cas 2> /dev/null | awk '{if ($7 > max) max = $7;} END{print (max/60)*1.04}'`
ymax2=`cat $file_imm $file_gp2 $file_rng $file_geo $file_log $file_cas 2> /dev/null | awk '{if ($2 > max) max = $2;} END{print (max/60)*1.04}'`
...
#
#    set out '${outfolder}/allmethods-gthrputs.99.9perc.eps'
#    set yrange [0:$ymax5]
#    plot \
